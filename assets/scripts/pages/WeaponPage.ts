...
const RED_400    = new Color(248, 113, 113, 255);

const THEME_COLOR = new Color(126,  34, 206, 255);  // PURPLE #7e22ce
...
export class WeaponPage extends Component {
    ...
    private buildPage() {
        ...
        const imgBorder = this.makeRect('ImgBorder', imgInner, imgW, imgH - 8, 0, 0, AMBER_600);
        this.makeRect('ImgBg', imgBorder, imgW - 6, imgH - 14, 0, 0, SLATE_900);
        this.makeLabel(imgBorder, '[ 当前法器展示 ]', 14, 0, 0, SLATE_400, imgW - 20);
        curY -= imgH / 2;

        // ── 武器属性统计栏 ──
        const statsH = 36;
        curY -= 12 + statsH / 2;
        const statsOuter = this.makeRect('StatsBorder', this.node, containerW, statsH, 0, curY, AMBER_700);
        const statsBg = this.makeRect('StatsBg', statsOuter, containerW - 8, statsH - 8, 0, 0, AMBER_900);
        const statLabels = ['攻击力', '品质', '强化'];
        const statValues = ['285', '★★★★★', 'Lv.10'];
        const segW = (containerW - 8) / 3;
        for (let i = 0; i < 3; i++) {
            const sx = -((containerW - 8) / 2) + segW * i + segW / 2;
            this.makeLabel(statsBg, statLabels[i], 10, sx, 6, AMBER_400, segW - 8);
            this.makeLabel(statsBg, statValues[i], 12, sx, -8, AMBER_200, segW - 8);
        }
        curY -= statsH / 2;

        // ── 武器列表 ──
        const weapons: WeaponItem[] = [
            { name: '青锋剑', level: 10, rarity: 5, attack: 285 },
            { name: '雷神锤', level: 8,  rarity: 4, attack: 256 },
            { name: '玄武盾', level: 7,  rarity: 4, attack: 198 },
            { name: '赤焰扇', level: 6,  rarity: 3, attack: 167 },
        ];
        ...
        for (let i = 0; i < weapons.length; i++) {
            itemY -= itemH / 2;
            this.buildWeaponItem(listBg, innerW, itemH, 0, itemY, weapons[i]);
            itemY -= itemH / 2 + itemGap;
        }
    }

    private buildWeaponItem(parent: Node, w: number, h: number, x: number, y: number, weapon: WeaponItem) {
        const item = this.makeRect('Weapon_' + weapon.name, parent, w, h, x, y, AMBER_950);
        ...
        this.makeLabel(item, weapon.name, 14, infoCX, 10, AMBER_100, infoW, HorizontalTextAlignment.LEFT);
        const stars = '★'.repeat(weapon.rarity);
        this.makeLabel(item, 'Lv.' + weapon.level + '  ' + stars, 11, infoCX, -6, YELLOW_400, infoW, HorizontalTextAlignment.LEFT);
        this.makeLabel(item, '攻击: ' + weapon.attack, 11, infoCX, -20, RED_400, infoW, HorizontalTextAlignment.LEFT);

        const btnX = w / 2 - pad - btnW / 2;
        const btnH = 28;
        const btn = this.makeRect('Btn', item, btnW, btnH, btnX, 0, THEME_COLOR);
        this.makeLabel(btn, '装备', 12, 0, 0, AMBER_100, btnW - 8);
    }
    ...
    private buildTitleBanner(parent: Node, w: number, h: number, x: number, y: number) {
        ...
        this.makeLabel(bg, '法器宝库', 16, 0, 0, AMBER_200, w - 96);
