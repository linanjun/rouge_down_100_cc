...
const THEME_COLOR = new Color(194,  65,  12, 255);  // ORANGE #c2410c
...
export class ShopPage extends Component {
    ...
    private buildPage() {
        ...
        this.makeLabel(imgBox, '[ 仙缘商店 ]', 14, 0, 0, SLATE_400, containerW - 40);
        curY -= imgH / 2;

        // ── 商品网格 (2列) ──
        const shopItems: ShopItem[] = [
            { name: '灵石礼包', price: 100, currency: '钻石', stock: 99 },
            { name: '经验丹',   price: 50,  currency: '金币', stock: 50 },
            { name: '强化石',   price: 80,  currency: '金币', stock: 30 },
            { name: '复活丹',   price: 200, currency: '钻石', stock: 10 },
        ];

        const gridH = h / 2 + curY - PAD - 16;
        curY -= 16 + gridH / 2;
        const gridOuter = this.makeRect('GridBorder', this.node, containerW, gridH, 0, curY, AMBER_700);
        const gridBg = this.makeRect('GridBg', gridOuter, containerW - 8, gridH - 8, 0, 0, AMBER_900);

        const colGap = 12;
        const innerW = containerW - 8 - 24;
        const cellW = (innerW - colGap) / 2;
        const cellH = (gridH - 8 - 24 - 12 * 1) / 2;
        const rowGap = 12;

        for (let i = 0; i < shopItems.length; i++) {
            const col = i % 2;
            const row = Math.floor(i / 2);
            const cx = col === 0 ? -(cellW + colGap) / 2 : (cellW + colGap) / 2;
            const cy = (gridH - 8) / 2 - 12 - cellH / 2 - row * (cellH + rowGap);
            this.buildShopItem(gridBg, cellW, cellH, cx, cy, shopItems[i]);
        }
    }

    private buildShopItem(parent: Node, w: number, h: number, x: number, y: number, item: ShopItem) {
        const cell = this.makeRect('Shop_' + item.name, parent, w, h, x, y, AMBER_950);
        this.drawBorder(cell, w, h, AMBER_800, 2);

        const pad = 10;
        const iconSize = 50;
        const iconY = h / 2 - pad - iconSize / 2;

        const iconBd = this.makeRect('IconBd', cell, iconSize, iconSize, 0, iconY - 6, AMBER_600);
        this.makeRect('IconBg', iconBd, iconSize - 6, iconSize - 6, 0, 0, SLATE_900);
        this.makeLabel(iconBd, '🎁', 20, 0, 0, AMBER_400, iconSize - 10);

        const nameY = iconY - iconSize / 2 - 18;
        this.makeLabel(cell, item.name, 14, 0, nameY, AMBER_100, w - pad * 2);

        const priceY = nameY - 20;
        this.makeLabel(cell, item.price + ' ' + item.currency, 12, 0, priceY, YELLOW_400, w - pad * 2);

        const stockY = priceY - 18;
        this.makeLabel(cell, '库存: ' + item.stock, 10, 0, stockY, SLATE_300, w - pad * 2);

        const btnW = w - pad * 2;
        const btnH = 28;
        const btnY = -h / 2 + pad + btnH / 2;
        const btn = this.makeRect('Btn', cell, btnW, btnH, 0, btnY, THEME_COLOR);
        this.makeLabel(btn, '购买', 12, 0, 0, AMBER_100, btnW - 8);
    }
    ...
    private buildTitleBanner(parent: Node, w: number, h: number, x: number, y: number) {
        ...
        this.makeLabel(bg, '仙缘商店', 16, 0, 0, AMBER_200, w - 96);
