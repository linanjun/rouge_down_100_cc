...
const THEME_COLOR = new Color(185,  28,  28, 255);  // RED #b91c1c
...
@ccclass('ForgePage')
export class ForgePage extends Component {
    ...
    private buildPage() {
        ...
        this.makeLabel(imgBox, '[ 炼器熔炉 ]', 14, 0, 0, SLATE_400, containerW - 40);
        curY -= imgH / 2;

        // ── 炼器列表 ──
        const forgeItems: ForgeItem[] = [
            { name: '玄铁重剑', level: 5, nextLevel: 6, materials: '精铁x10' },
            { name: '金丝软甲', level: 3, nextLevel: 4, materials: '金线x8' },
            { name: '护身玉佩', level: 2, nextLevel: 3, materials: '玉石x5' },
        ];
        ...
        itemY -= 12;
        for (let i = 0; i < forgeItems.length; i++) {
            itemY -= itemH / 2;
            this.buildForgeItem(listBg, innerW, itemH, 0, itemY, forgeItems[i]);
            itemY -= itemH / 2 + itemGap;
        }
    }

    private buildForgeItem(parent: Node, w: number, h: number, x: number, y: number, forge: ForgeItem) {
        const item = this.makeRect('Forge_' + forge.name, parent, w, h, x, y, AMBER_950);
        ...
        this.makeLabel(item, forge.name, 14, infoCX, 12, AMBER_100, infoW, HorizontalTextAlignment.LEFT);
        this.makeLabel(item, 'Lv.' + forge.level + ' → Lv.' + forge.nextLevel, 11, infoCX, -4, YELLOW_400, infoW, HorizontalTextAlignment.LEFT);
        this.makeLabel(item, '需要: ' + forge.materials, 11, infoCX, -20, SLATE_300, infoW, HorizontalTextAlignment.LEFT);

        const btnX = w / 2 - pad - btnW / 2;
        const btnH = 28;
        const btn = this.makeRect('Btn', item, btnW, btnH, btnX, 0, THEME_COLOR);
        this.makeLabel(btn, '强化', 12, 0, 0, AMBER_100, btnW - 8);
    }
    ...
    private buildTitleBanner(parent: Node, w: number, h: number, x: number, y: number) {
        ...
        this.makeLabel(bg, '炼器坊', 16, 0, 0, AMBER_200, w - 96);
