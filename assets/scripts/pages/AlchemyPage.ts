...
const THEME_COLOR = new Color(  4, 120,  87, 255);  // EMERALD #047857
...
export class AlchemyPage extends Component {
    ...
    private buildPage() {
        ...
        this.makeLabel(imgBox, '[ 炼丹炉 ]', 14, 0, 0, SLATE_400, containerW - 40);
        curY -= imgH / 2;

        // ── 丹药列表 ──
        const pills: PillItem[] = [
            { name: '筑基丹', materials: '灵草x5',  time: '2小时' },
            { name: '洗髓丹', materials: '灵芝x3',  time: '4小时' },
            { name: '金丹',   materials: '仙草x10', time: '8小时' },
        ];
        ...
        for (let i = 0; i < pills.length; i++) {
            itemY -= itemH / 2;
            this.buildPillItem(listBg, innerW, itemH, 0, itemY, pills[i]);
            itemY -= itemH / 2 + itemGap;
        }
    }

    private buildPillItem(parent: Node, w: number, h: number, x: number, y: number, pill: PillItem) {
        const item = this.makeRect('Pill_' + pill.name, parent, w, h, x, y, AMBER_950);
        ...
        this.makeLabel(item, pill.name, 14, infoCX, 12, AMBER_100, infoW, HorizontalTextAlignment.LEFT);
        this.makeLabel(item, '材料: ' + pill.materials, 11, infoCX, -4, AMBER_400, infoW, HorizontalTextAlignment.LEFT);
        this.makeLabel(item, '耗时: ' + pill.time, 11, infoCX, -20, SLATE_300, infoW, HorizontalTextAlignment.LEFT);

        const btnX = w / 2 - pad - btnW / 2;
        const btnH = 28;
        const btn = this.makeRect('Btn', item, btnW, btnH, btnX, 0, THEME_COLOR);
        this.makeLabel(btn, '炼制', 12, 0, 0, AMBER_100, btnW - 8);
    }
    ...
    private buildTitleBanner(parent: Node, w: number, h: number, x: number, y: number) {
        ...
        this.makeLabel(bg, '炼丹房', 16, 0, 0, AMBER_200, w - 96);
