...
const THEME_COLOR = new Color( 21, 128,  61, 255);  // GREEN #15803d
...
export class TaskPage extends Component {
    ...
    private buildPage() {
        ...
        this.makeLabel(imgBox, '[ 任务大厅 ]', 14, 0, 0, SLATE_400, containerW - 40);
        curY -= imgH / 2;

        // ── 任务列表 ──
        const tasks: TaskItem[] = [
            { name: '击败妖兽', reward: '经验+100', status: '进行中', progress: 5, total: 10 },
            { name: '收集灵草', reward: '金币+50',  status: '进行中', progress: 3, total: 5 },
            { name: '炼制丹药', reward: '钻石+10',  status: '未完成', progress: 0, total: 3 },
            { name: '修炼功法', reward: '功法点+20', status: '可领取', progress: 5, total: 5 },
        ];
        ...
        for (let i = 0; i < tasks.length; i++) {
            itemY -= itemH / 2;
            this.buildTaskItem(listBg, innerW, itemH, 0, itemY, tasks[i]);
            itemY -= itemH / 2 + itemGap;
        }
    }

    private buildTaskItem(parent: Node, w: number, h: number, x: number, y: number, task: TaskItem) {
        const item = this.makeRect('Task_' + task.name, parent, w, h, x, y, AMBER_950);
        ...
        this.makeLabel(item, task.name, 14, infoCX, 16, AMBER_100, infoW, HorizontalTextAlignment.LEFT);
        this.makeLabel(item, '奖励: ' + task.reward, 11, infoCX, 0, AMBER_400, infoW, HorizontalTextAlignment.LEFT);

        // 进度条
        const barW = infoW;
        const barH = 8;
        const barBg = this.makeRect('BarBg', item, barW, barH, infoCX, -16, SLATE_900);
        ...
        this.makeLabel(item, task.progress + '/' + task.total, 10, infoCX, -28, SLATE_300, infoW, HorizontalTextAlignment.LEFT);

        // 状态按钮
        const btnX = w / 2 - pad - btnW / 2;
        const btnH = 28;
        const btnColor = task.status === '可领取' ? THEME_COLOR : SLATE_700;
        const btn = this.makeRect('Btn', item, btnW, btnH, btnX, 0, btnColor);
        this.makeLabel(btn, task.status, 11, 0, 0, AMBER_100, btnW - 8);
    }
    ...
    private buildTitleBanner(parent: Node, w: number, h: number, x: number, y: number) {
        ...
        this.makeLabel(bg, '任务大厅', 16, 0, 0, AMBER_200, w - 96);
